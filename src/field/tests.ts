...
import { Field, fieldChangeType } from "./index.tsx";
  ...
  describe("watching", () => {
    ...
    describe("trigger", () => {
      it("triggers the watchers", () => {
        ...
        const spy = vi.fn();
        field.watch(spy);
        field.trigger(stateChangeType.value);
        expect(spy).toHaveBeenCalledWith(
          42,
          expect.objectContaining({ detail: stateChangeType.value })
        );
      ...
      it("doesn't trigger parent fields", () => {
        ...
        const spy = vi.fn();
        field.watch(spy);
        field.$.num.trigger(stateChangeType.value);
        expect(spy).not.toHaveBeenCalled();
      });
      ...
      it("allows to notify parent fields", () => {
        ...
        const spy = vi.fn();
        field.watch(spy);
        field.$.num.trigger(stateChangeType.value, true);
        expect(spy).toHaveBeenCalledWith(
          { num: 42 },
  ...
  describe("field", () => {
    describe("register", () => {
      it("generates props for a field", () => {
        ...
        const props = field.$.name.$.first.register();
    ...
    describe("errors", () => {
      ...
      describe("errors", () => {
        it("collects map all children errors", () => {
          ...
          expect(errors.get(field)).toEqual({ message: "Something is wrong" });
          expect(errors.get(field.$.age)).toEqual({
            message: "Are you an immortal?",
          });
          expect(errors.get(field.$.name.$.first)).toEqual({
            message: "First name is required",
          });
          expect(errors.get(field.$.ids.$(1))).toEqual({
